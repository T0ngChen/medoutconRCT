...
fit_treat_mech_RCT = function(
  ...
  if (is.null(valid_data)) {
    treat_pred_A_prime <- contrast[1] *
      treat_pred +
      (1 - contrast[1]) * (1 - treat_pred)
    treat_pred_A_star <- contrast[2] *
      treat_pred +
    ...
    data.table::setnames(
      out_treat_est,
      c("treat_pred_A_prime", "treat_pred_A_star")
    )
    out <- list(treat_est = out_treat_est, treat_fit = treat_fit)
  } else {
    out_treat_est <- lapply(list(train_data, valid_data), function(data) {
      treat_task <- sl3::sl3_Task$new(
        data = data,
        weights = "obs_weights",
        covariates = cov_names,
...
fit_out_mech_RCT = function(
  ...
  if (is.null(valid_data)) {
    ...
    data.table::setnames(
      out_b_est,
      c("b_pred_A_natural", "b_pred_A_prime", "b_pred_A_star")
    )
    ...
    out_b_est <- lapply(
      ...
      function(data_intervene) {
        ...
        b_intervened_pred_A_prime <- b_natural_fit$predict(
          b_intervened_prime_task
        )
        ...
        b_intervened_pred_A_star <- b_natural_fit$predict(
          b_intervened_star_task
        )
...
fit_moc_mech_RCT = function(
  ...
  if (is.null(valid_data)) {
    ...
    data.table::setnames(
      out_moc_est,
      c("moc_pred_A_natural", "moc_pred_A_prime", "moc_pred_A_star")
    )
    out <- list(moc_est = out_moc_est, moc_fit = moc_fit)
  } else {
    train_data_intervene <- data.table::copy(train_data)
    valid_data_intervene <- data.table::copy(valid_data)
    ...
    out <- list(
      moc_est_train_M_one = out_moc_est[[1]],
      moc_est_valid_M_one = out_moc_est[[2]],
      moc_est_train_M_natural = out_moc_est[[1]] *
        train_data$M +
        (1 - out_moc_est[[1]]) * (1 - train_data$M),
      moc_est_valid_M_natural = out_moc_est[[2]] *
        valid_data$M +
...
fit_nuisance_u_RCT = function(
  ...
  q_star_M_natural <- q_out$moc_est_train_M_natural$moc_pred_A_star[
    train_data$R == 1
  ]
  r_prime_M_natural <- r_out$moc_est_train_M_natural$moc_pred_A_prime
  train_data <- train_data[R == 1, ]
  valid_data <- valid_data[R == 1, ]
  d_star <- (g_star / g_prime) *
    (q_star_M_natural / r_prime_M_natural) *
    (h_prime / h_star)
...
fit_nuisance_v_RCT = function(
  ...
  if (effect_type == "shift_k") {
    q_train_star_M_one <- q_out$moc_est_train_M_one$moc_pred_A_star[
      train_data$R == 1
    ]
    q_valid_star_M_one <- q_out$moc_est_valid_M_one$moc_pred_A_star[
      valid_data$R == 1
    ]
    train_data <- train_data[R == 1, ]
    valid_data <- valid_data[R == 1, ]
    ...
    v_pseudo <- lapply(unique(train_data$M), function(m_val) {
      ...
      out <- list(
        training = out_train,
        validation = out_valid,
        b_train = b_pred_train_m_interv,
        b_valid = b_pred_valid_m_interv
      )
      return(out)
    })
...
fit_nuisance_l_RCT = function(
  ...
  var_names = c(w_names, z_names)
  l_data_train <- data.table::as.data.table(cbind(
    train_data[, ..var_names],
    train_data$A,
    train_data$M,
    b_prime,
  ...
  l_data_valid <- data.table::as.data.table(cbind(
    valid_data[, ..var_names],
    valid_data$A,
    valid_data$M,
    rep(0, nrow(valid_data)),
  ...
  return(list(
    l_fit = l_param_fit,
    l_pred = as.numeric(l_valid_pred),
