...
medoutconRCT = function(W,
  ...
  if (is.null(L)) {
    ...
    l_names <- paste("L", seq_len(dim(data.table::as.data.table(L))[2]),
                     sep = "_")
  }
  ...
  data.table::setnames(data, c(
    "Y", "M", "R", z_names, l_names, "A", w_names,
    "obs_weights", "two_phase_weights"))
  ...
  est_params_out <- lapply(contrast_grid, function(contrast) {
    if (estimator == "onestep") {
      ...
      onestep_est_args_out <- list(data = data_out,
                                   contrast = contrast,
                                   g_learners = g_learners,
                                   h_learners = h_learners,
                                   b_learners = b_learners,
                                   ...
                                   svy_weights = svy_weights,
                                   g_bounds = g_bounds)
      onestep_est_args_out <- unlist(list(onestep_est_args_out,
                                      est_args_os), recursive = FALSE)
      ...
      data_out[, (z_names) := lapply(.SD, function(x) 1), .SDcols = z_names]
      data_out[, (l_names) := lapply(.SD, function(x) 1), .SDcols = l_names]
      tmle_est_args_out <- list(data = data_out,
                                contrast = contrast,
                                g_learners = g_learners,
                                h_learners = h_learners,
                                b_learners = b_learners,
                                ...
                                svy_weights = svy_weights,
                                g_bounds = g_bounds)
      tmle_est_args_out <- unlist(list(tmle_est_args_out, est_args_tmle),
                              recursive = FALSE)
  ...
  de_eif_org <- est_params_out[[1]]$eif_est_rescaled - est_params[[1]]$eif_est_rescaled
  de_var_est <- stats::var(de_eif_est)/nrow(data)
  de_est_out <- list(theta = de_theta_est,
                     var = de_var_est,
                     eif = de_eif_est,
                     type = estimator,
...
medoutconPall = function(W,
  ...
  est_params_out <- lapply(contrast_grid, function(contrast) {
    if (estimator == "onestep") {
      ...
      data_out <- data.table::copy(data)
      data_out[, `:=`(M = 1)]
      onestep_est_args <- list(data = data_out,
                               contrast = contrast,
                               g_learners = g_learners,
                               h_learners = h_learners,
                               b_learners = b_learners,
                               ...
                               svy_weights = svy_weights,
                               g_bounds = g_bounds)
      onestep_est_args <- unlist(list(onestep_est_args,
                                      est_args_os), recursive = FALSE)
      est_out <- do.call(est_onestep_RCT, onestep_est_args)
    } else if (estimator == "tmle") {
      # TARGETED MINIMUM LOSS ESTIMATOR
      data_out <- data.table::copy(data)
      data_out[, `:=`(M = 1)]
      tmle_est_args <- list(data = data_out,
                            contrast = contrast,
                            g_learners = g_learners,
                            h_learners = h_learners,
                            b_learners = b_learners,
